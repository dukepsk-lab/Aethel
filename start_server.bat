...
REM
REM  Prerequisites (one-time setup — see README):
...
REM    5. pip install -e ".[mt5]"  (run once in this folder)
REM ============================================================
...
if errorlevel 1 (
    ...
    net start postgresql-x64-16 2>nul || net start postgresql 2>nul
...
if exist "frontend\package.json" (
    ...
    start "Aethel Frontend" /min cmd /c "cd frontend && npm run build >nul 2>&1 && npm start"
    echo       Frontend starting at http://localhost:3000 (may take ~30s)
) else (
